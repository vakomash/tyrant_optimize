	...
	while (true)
	{
		...
		while (true)
		{
			...
			else
			{
				...
				shared_mutex.unlock(); //>>>>
				std::vector<Results<uint64_t>> result{sim.evaluate()};				//calculate single sim
				shared_mutex.lock(); //<<<<
				std::vector<uint64_t> thread_score_local(thread_results->first.size(), 0u); //!
				for (unsigned index(0); index < result.size(); ++index)
				{
					thread_results->first[index] += result[index]; //! 				//index refers to different enemy_hands/decks += look sim.h, sums results
					thread_score_local[index] = thread_results->first[index].points; //!	//thread_score_local only for compare() call
				}
				++thread_results->second; //!							//iterations count
				unsigned thread_total_local{thread_results->second}; //!
				shared_mutex.unlock(); //>>>>
				if (thread_compare && thread_id == 0 && thread_total_local > 1) //check if score of current simmed deck can be better than best score anymore
				{
